...
async function sendOrderStatusSMS(order) {
  ...
  try {
    ...
    // مانوقفش الطلب لو فشل إرسال SMS - بس نسجل الخطأ في اللوج
...
// كل يوم الساعة 3 صباحًا (بتوقيت السيرفر)
cron.schedule('0 3 * * *', runBackupJob);


// لو الـ JSON اللي جاي من العميل تالف/مش صحيح، نرجع خطأ واضح بدل ما السيرفر يقع
app.use((err, req, res, next) => {
  if (err.type === 'entity.parse.failed') return res.status(400).json({ error: 'بيانات غير صالحة' });
  next(err);
});

// حماية بسيطة من إغراق السيرفر بطلبات كتير من نفس المصدر (IP) في وقت قصير
const writeLimiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 دقيقة
  max: 120, // أقصى 120 عملية كتابة لكل IP كل 15 دقيقة
...
const orderLimiter = rateLimit({
  ...
  max: 15, // أقصى 15 طلب شراء لكل IP كل 10 دقايق - يمنع سبام الطلبات الوهمية
  standardHeaders: true,
  legacyHeaders: false,
...
// ---------- الموديلات (Schemas) ----------
// كل موديل فيه حقل "id" نصي هو نفسه الـ id اللي الفرونت إند بيولّده (زي 'prod_'+Date.now())
// عشان نفس شكل البيانات القديم يفضل شغال من غير ما نغيّر منطق الفرونت إند
...
app.put('/api/categories/reorder', writeLimiter, wrap(async (req, res) => {
  const arr = req.body; // مصفوفة الأقسام بترتيبها الجديد
...
app.post('/api/orders', orderLimiter, wrap(async (req, res) => {
  ...
  // تحقق أساسي من البيانات قبل ما نلمس المخزون خالص
  ...
  // الخطوة 1: خصم المخزون بشكل آمن (atomic) - يمنع بيع نفس القطعة مرتين لعميلين في نفس اللحظة
  const decremented = [];
  for (const item of items) {
    ...
    if (!updated) {
      // تراجع فوري عن أي خصم سابق حصل في نفس الطلب
  ...
  // الخطوة 2: فحص أقصى كمية لكل عميل بناءً على طلباته الفعلية السابقة (مش بس السلة الحالية في متصفحه)
  for (const item of items) {
  ...
  // الخطوة 3: توليد رقم طلب فريد فعليًا من السيرفر (مش من متصفح العميل) لتجنب أي تطابق نادر
...
app.patch('/api/orders/:id', writeLimiter, wrap(async (req, res) => {
  ...
  if (statusChanged) sendOrderStatusSMS(order); // fire-and-forget، ما بيأخرش الرد على الأدمن
  res.json(order.toJSON());
}));
...
// upsert بناءً على رقم التليفون (نفس منطق upsertCustomer في الفرونت إند)
...
// تشغيل نسخة احتياطية يدويًا (للتجربة) - محمي بكلمة سر بسيطة عن طريق الهيدر x-backup-secret
...
app.listen(PORT, () => console.log(`✓ ACS backend running on port ${PORT}`));
